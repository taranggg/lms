...
interface BatchSectionProps {
    mode: "admin" | "trainer";
    token: string;
    userId?: string; // Trainer ID, required if mode is 'trainer'
}

export default function BatchSection({ mode, token, userId }: BatchSectionProps) {
    ...
    // Fetch Metadata (Branches/Trainers) - Mostly relevant for Admin filters
    useEffect(() => {
    ...
    const fetchBatches = async () => {
        ...
        try {
            ...
            if (mode === 'admin') {
                ...
                // For trainer, we might want to filter by their ID if the API supports/requires it
                // Or usually the API `getTrainerBatches` or similar is used.
                // Assuming `getAllBatches` handles filtering by trainer if passed or we filter client side?
                // Based on previous code, `getAllBatches` accepts filters.
    ...
    const getBranchName = (branchIdOrObj: any) => {
        ...
        // Only works if branches are loaded (Admin mode)
    ...
    return (
        <div className="flex flex-col gap-6 w-full relative min-h-[calc(100vh-150px)]">
             {/* Header & Controls */}
            <div className={`flex flex-col ${mode === 'admin' ? 'md:flex-row md:items-center justify-between' : ''} gap-4`}>
                <div>
