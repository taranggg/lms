...
export default function AdminTrainersPage() {
  ...
  const fetchTrainers = async () => {
    ...
    try {
      ...
      const trainersRes = await getAllTrainers(filters);
  ...
  const getBranchName = (branchIdOrObj: any) => {
    ...
    if (typeof branchIdOrObj === "object" && branchIdOrObj.name)
      return branchIdOrObj.name;
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer group/item">
                  <Phone
                    size={14}
                    className="text-green-500 group-hover/item:scale-110 transition-transform"
                  />
                  <span className="text-xs">
                    {trainer.mobileNumber || "N/A"}
                  </span>
                </div>
              </div>
