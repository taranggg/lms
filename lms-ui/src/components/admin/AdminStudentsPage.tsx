...
export default function AdminStudentsPage() {
  ...
  const availableBatches = React.useMemo(() => {
    ...
    return batches.filter(batch => {
        const bBranchId = typeof batch.branch === "string" ? batch.branch : batch.branch?._id;
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2">
                <Filter size={16} />
                <span>{filterBranch === "All" ? "Branch" : getBranchName(filterBranch)}</span>
              </Button>
            </DropdownMenuTrigger>
           ...
           <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2">
                ...
                <span>{filterBatch === "All" ? "Batch" : getBatchName(filterBatch)}</span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              ...
              {availableBatches.map((b) => (
                  <DropdownMenuItem key={b._id} onClick={() => setFilterBatch(b._id)}>
                      {b.name}
                  </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
            ...
            >
                ...
                <div className="flex gap-4 items-center">
                    ...
                    <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground mt-0.5">
                        ...
                        <span className="truncate max-w-[150px]">{getBatchName(student.batch)}</span>
                    </div>
                    </div>
                </div>
                <Button
